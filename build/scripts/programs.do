...
* Purpose: Script to run our own user written programs
* Created by: Nicolas Herrera L.
* Created on: 4/6/2020
* Last modified on: 4/13/2020
...
/*---------------------------------------------------------------
 FOLDER: Program to create the directory structure
****************************************************************/

/* The program works very simple: Type folder and after type the path
where you want to save the project with the name of the new project folder at
the end e.g create the project new in the documents directory will be just
folder C:/documents/new */

cap program drop folder
program define folder
    args namelist
...
/* If the main folder exists, display an error message */
if _rc display "Existent folder", as error

/* If the main folder do not exist, proceed */
if !_rc {

	/*---------------------------------------------------------------
   Create main subfolders
  ---------------------------------------------------------------*/
    /* Folder storing the construction of databases */
    mkdir "`1'\build"

    /* Folder storing the analysis of cleaned databases */
    mkdir "`1'\analysis"

    /* Folder storing the versions of the paper  */
    mkdir "`1'\drafts"

    /* Folder storing the documentation of the project*/
    mkdir "`1'\references"

    /*--------------------------------------------------------------
     Create common subfolders in the analysis and build stage
    ---------------------------------------------------------------*/

    /* Loop over the two main stages to create common subfolders */
    foreach stage in "\build" "\analysis" {

      /* Store the inputs of the stage */
      mkdir "`1'`stage'\inputs"

      /* Store the outputs of the stage */
      mkdir "`1'`stage'\outputs"

      /* Store the temporary/intermediary files of the stage */
      mkdir "`1'`stage'\temp"

      /* Store the scripts of the stage */
      mkdir "`1'`stage'\scripts"

      /* Store the log files of the stage */
      mkdir "`1'`stage'\logs"
  ...
  /*--------------------------------------------------------------
   Additional subfolders in the analysis stage
  ---------------------------------------------------------------*/

    /* Store the figures produced with cleaned data */
    mkdir "`1'\analysis\outputs\figures"

    /* Store the tables with cleaned data */
    mkdir "`1'\analysis\outputs\tables"
}

end

/*--------------------------------------------------------------
 to_log: Program to generate the log of a variable with formated
 name and label
---------------------------------------------------------------*/

cap program drop to_log
program to_log

  syntax varlist [if] [in]

  /* loop over variables in the varlist */
  foreach var of varlist `varlist'  {

    /* Save the variable's label in local */
    local lab: variable label `var'

    /* Change the variable's label to lower case for consistency */
    local lab = ustrlower("`lab'")

    /* Generate the log of the variable with the prefix ln in the name */
    g l_`var' = ln(`var')

    /* Label the variable */
    lab var l_`var' "Log of `lab'"

  }
end

/*--------------------------------------------------------------
 ineqdeco_large: Modification of the program ineqdeco. Allows using a large number of subgroups.
 Does not display the results but stores the results as scalars
---------------------------------------------------------------*/
cap program drop ineqdeco_large
program ineqdeco_large, sortpreserve rclass 
...
display "Modification of the program ineqdeco. Allows using a large number of subgroups. Does not display the results but stores the results as scalars"
